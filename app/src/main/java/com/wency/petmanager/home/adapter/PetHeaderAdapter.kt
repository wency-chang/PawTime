...
class PetHeaderAdapter(private val viewModel: HomeViewModel, private val homeFragment: HomeFragment):
    ListAdapter<Pet, PetHeaderAdapter.PetOptionViewHolder>(DiffCallback) {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PetOptionViewHolder {
        val layoutInflater = LayoutInflater.from(parent.context)

        return PetOptionViewHolder(
            ItemPetHeaderBinding.inflate(layoutInflater, parent, false))
    }
    ...
    override fun onBindViewHolder(holder: PetOptionViewHolder, position: Int) {
            ...
            holder.petHeader.setOnClickListener {
                viewModel.queryByPet(position,
                    viewModel.petQueryPosition.value != position)
